...
export default function Enquiries() {
  ...
  const createEnquiryMutation = useMutation({
    mutationFn: (data) => apiRequest('/api/enquiries', {
      method: 'POST',
  ...
  const updateEnquiryMutation = useMutation({
    mutationFn: ({ id, data }) => apiRequest(`/api/enquiries/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
    onSuccess: () => {
  ...
  const deleteEnquiryMutation = useMutation({
    mutationFn: (id) => apiRequest(`/api/enquiries/${id}`, {
      method: 'DELETE',
    }),
    onSuccess: () => {
      queryClient.invalidateQueries(['/api/enquiries']);
        ...
        <TabsContent value="list">
          <Card>
            <CardHeader>
              ...
              <CardDescription>View and manage all visa enquiries</CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                <div className="flex justify-center py-8">Loading enquiries...</div>
                    ...
                    <TableBody>
                      {enquiries.map((enquiry) => (
                        <TableRow key={enquiry._id}>
                          ...
                          <TableCell>{enquiry.intendedTravelDate ? new Date(enquiry.intendedTravelDate).toLocaleDateString() : '-'}</TableCell>
                          <TableCell>
                            <div className="flex space-x-2">
                              <Button variant="outline" size="sm" onClick={() => handleEdit(enquiry)}>Edit</Button>
                              <Button variant="destructive" size="sm" onClick={() => handleDelete(enquiry._id)}>Delete</Button>
                            </div>
                          </TableCell>
                        </TableRow>
                      ))}
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500">No enquiries found. Create your first enquiry!</p>
                  <Button className="mt-4" onClick={() => setActiveTab('create')}>Create Enquiry</Button>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="create">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Add a new visa enquiry to the system</CardDescription>
            </CardHeader>
            <CardContent>
